...
namespace OneSkyBlock
{
    public class OneSkyBlockTile : GlobalTile
    {
        ...
        // Disable mining sound for skyblock
        ...
        public override void KillTile(int i, int j, int type, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
            ...
            for (int p = 0; p < playerCount; p++)
            {
                ...
                if (i == (Main.spawnTileX + (p * 5)) && j == Main.spawnTileY && fail == false)
                {
                    ...
                    if (NPC.downedBoss2) // Unlockable hardmode ores 
                    {
                    ...
                    if (NPC.downedBoss2 && NPC.downedBoss3) // EoW or BoC and Skeletron defeated drops
                    {
                    ...
                    if (NPC.downedBoss2 && NPC.downedBoss3 && Main.hardMode) // ^ Hardmode enabled
                    {
                    ...
                    if (Main.hardMode && NPC.downedMechBossAny) // Any mechanical boss defeated drops
                    {
                    ...
                    if (Main.hardMode && NPC.downedPlantBoss) // Plantera defeated drops
                    {
                    ...
                    if (Main.hardMode && NPC.downedGolemBoss) // Golem defeated drops
                    {
                        skyDrops.AddEntry(ItemID.PlatinumCoin, 0.01 * ModSkyBlockConfig.OneBlockItemsRate);
                    }
                    if (Main.hardMode && NPC.downedMoonlord) // Moonlord defeated drops
                    {
                    ...
                    if (ModSkyBlockConfig.OneBlockOreRate > 0) // Ore drop rate pity and reset
                    {
    ...
    public class OneSkyBlockDrop : GlobalItem
    {
        ...
        {
            // Add Lihzard Alter to plantera loot
    ...
    {
        // Extractinator added to traveling merchant
    ...
    {
        // Option for multiple cloud tiles for multiplayer
